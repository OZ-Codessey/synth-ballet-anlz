...
**관찰(Fact)** : 그랑 제떼 높이는 연간 30\~33cm 구간에서 등락을 반복하며, 뚜렷한 상승 추세는 관찰되지 않는다. **실제 측정된 281일 기준 평균은 31.82cm, 표본 표준편차는 3.02cm**이다. STL·ARIMA처럼 연속 시계열이 필요한 분석에서는 결측 84일을 선형보간한 365일 시계열을 사용하며, 이 보간 시계열의 평균은 약 31.63cm, 표준편차는 약 3.20cm이다. 부상 구간(8/18\~9/26)에서 명확한 하락이 나타난다.
...
**결론** : H4는 **지지**되었다. 서로 단위가 다른 3가지 지표를 정규화 하고 부상 전 상황 수준을 100으로 기준선을 잡고 비교하였다. 수치상으로는 큰 턴아웃(각도)이 실제로는 미미한 차이가 있고, 수치상으로는 작은 숫자지만 정규화한 후 비교시, 를르베가 부상 시 훨신 큰 데미지가 있음을 확인했다. 이는 발목 부상과 직결되는 지표이기 때문으로 보인다. 를르베와 제때는 부상 중 기술지표가 크게 하락했고, 회복 완료 후에는 부상 전 수준에 근접했지만 뚜렷하게 초과하지는 않았다.
<br>
<br>
